...
- `*.myworkdayjobs.com` → Workday (**account wall + multi-page wizard, assisted**)
...
- **Workday**: every company requires a separate account - handle it per "Account walls" (the agent registers/signs in itself via `credentials.json`). Then a 5-7 page wizard (My Information → My Experience → questions → Voluntary Disclosures → Self-Identify → Review); CAPTCHAs common (user solves them). The agent fills each page; the user clicks every "Next" and the final "Submit". Never attempt the whole wizard in one unattended pass.
